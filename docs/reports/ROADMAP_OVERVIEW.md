...
- **M5 Drift Decoupling: Engine & Verbs** (one verb remaining, `drift enrich`): the config-driven engine/integration split; the two tasks that consumed engine output in `src/` (retired/deployed axes, timeline visuals) moved to M8 as aesthetic follow-on rather than coupling work.
- **M6 Drift: Tests & Docs**: lock the stable engine down; gated on M3 and M5, so with M3 done it now waits on `drift enrich` alone.
