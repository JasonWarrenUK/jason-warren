...
import { renderOgCard } from '$lib/og/card.js';
import { DEFAULT_DESCRIPTION } from '$lib/config.js';
...
function truncate(text: string, max = 116): string {
...
export const GET: RequestHandler = async ({ params }) => {
	const card =
		params.slug === 'default'
			? { eyebrow: 'Developer', title: 'Jason Warren', subtitle: truncate(DEFAULT_DESCRIPTION) }
			: (() => {
					const project = getBySlug(params.slug as ProjectSlug);
					if (!project) return null;
					return { eyebrow: 'Project', title: project.name, subtitle: truncate(project.tagline) };
				})();
