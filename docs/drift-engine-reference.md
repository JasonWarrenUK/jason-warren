...
| `author.botPattern`   | `string` (ERE alternation)     | `'\\[bot\\]\|github-actions'` | Non-human authors removed from the all-authors commit **denominator only** (churn/LOC still count every commit); extend with AI-agent identities if agents commit under their own identity | `check-drift.js` → `BOT_PATTERN`                                 |
...
`author.botPattern`, `excludedRepoNames`, and `theme.*`, but omits `files`.
...
`author.botPattern`'s scaffolded default (`'\\[bot\\]|github-actions'`) is
generic, not Jason-specific: the AI-agent identity previously baked into
`DEFAULTS` (`noreply@anthropic.com`) now lives only in this portfolio's own
`drift.config.ts`. A fresh `drift init` on someone else's machine scaffolds a
sensible default and a comment showing how to extend it, rather than silently
inheriting Jason's identity. Fixed in roadmap task `5DR.26`.
...
Step 2 scaffolds `author.botPattern` with a generic CI-bot default; extend it
by hand in `drift.config.ts` if this machine's commit history includes
AI-agent authors you want excluded from the all-authors count too.
