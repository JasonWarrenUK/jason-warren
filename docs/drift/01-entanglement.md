...
| Five enum and field lists are hand-mirrored from `src/lib/data/types.ts` (`AUTHOR_FIELD_ENUMS`, the two relationship-kind sets, `TECH_TAG_KINDS`, `TECH_SURFACES`, `AUTHORED_FIELDS`); ADR-001 moves the contract into `scripts/overlay.schema.json` with a parity-tested `.d.ts`, and derives these lists from it | 10EX.7     |
...
| The nine overlay verbs assume `config.paths.projects` and its three sibling files exist; an Engine-only adopter has no overlay directory and gets a stack trace, not a message                                                                                                                                     | 10EX.9     |
